import normal from "./normal";

export default {
  borderLarge: normal.borderLarge * 2,
  borderMedium: normal.borderMedium * 2,
  borderSmall: normal.borderSmall * 2,
  borderRadiusLarge: normal.borderRadiusLarge * 2,
  borderRadiusMedium: normal.borderRadiusMedium * 2,
  borderRadiusSmall: normal.borderRadiusSmall * 2,
  gapLarge: normal.gapLarge * 2,
  gapMedium: normal.gapMedium * 2,
  gapSmall: normal.gapSmall * 2,
  navSize: normal.navSize * 2,
  spinnerSize: normal.spinnerSize * 2,
  maxCategoryWidth: normal.maxCategoryWidth * 2,
  maxCategoryHeight: normal.maxCategoryHeight * 2,
  maxChannelWidth: normal.maxChannelWidth * 2,
  maxChannelHeight: normal.maxChannelHeight * 2,
};
